#' Master Facility List Google Sheet ID
#'
#' @export
#' @return google sheet id
#' @keywords internal
#'

mfl_new_id <- "1UuDYK4X-Lr8avq4IUsKGWChr17AE4Bc21MlHO3OWB-Y"

#' FY24 Master Facility List Google Sheet ID
#'
#' @export
#' @return google sheet id
#' @keywords internal

mfl_fy24_id <- "1whfC1a9Aga6UZNleuNO8n4orLUeeuu5Ew59OpnXFqwc"

#' FY23 Master Facility List Google Sheet ID
#'
#' @export
#' @return google sheet id
#' @keywords internal
#'

mfl_fy23_id <- "1QWVeT2wXA428WLYBy6-X5vP9A6sXjLjdtmkvLTw3mJA"


#'USAID Disaggregate Mapping File Google Sheet ID
#'
#' @export
#' @return google sheet id
#' @keywords internal
#'
new_disagg_map_id <- "1oyWFm_yWmurWdBQ0dZh4__VuuyjPvox56yO5FpJuwHE"
