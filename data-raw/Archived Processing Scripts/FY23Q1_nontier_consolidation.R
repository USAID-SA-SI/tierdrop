...
FY23Q2_Master_NonTier_DRAFT<- bind_rows(ANOVA,
                                        MATCH,
                                        BRCH, RTC, WRHI_70301, WRHI_80007) %>%
...
write_csv(FY23Q1_Master_NonTier_DRAFT, glue("{dataout}/FY23Q1_USAID_SA_Import_NonTier_consolidated_v1_{today}.csv"))
...
tier_final <- dataout %>%
  return_latest("FY23Q1_TIER_Import_File_v4_2023-02-03") %>%
  read_csv()

non_tier_final <- dataout %>%
  return_latest("FY23Q1_NonTier_consolidated_v1_2023-02-02") %>%
  read_csv()


fy23q1_final <- bind_rows(tier_final, non_tier_final)

fy23q1_nozero <- fy23q1_final %>% filter(value != 0)

#write master consolidated
write_csv(fy23q1_final, glue("{dataout}/FY23Q1_USAID_SA_Import_FINALv2_{today}.csv"))
write_csv(fy23q1_nozero, glue("{dataout}/FY23Q1_USAID_SA_Import_FINALv2_{today}_NOZERO.csv"))
