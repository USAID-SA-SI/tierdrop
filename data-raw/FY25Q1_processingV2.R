...
get_meta("FY25Q1")
fiscal_quarter <- "FY25Q1"


#set folderpaths (@to-do turn this into a get_metadata() function later)
ndoh_filepath <- ndoh_folderpath %>% glamr::return_latest("MER")
...
mfl_new_df <- googlesheets4::read_sheet(mfl_fy25_id, sheet = "MFL_FY25_Q1")
...
df_fac <- clean_mfl(mfl_period ="FY25Q1") %>%
...
ndoh_all <- import_ndoh(filepath = ndoh_filepath, qtr = curr_qtr, kp = FALSE) %>%
  mutate(`Test Result/Outcome/Duration`=case_when(`Test Result/Outcome/Duration` %in%
                                                    c("IIT 36+","IIT 6-12","IIT 13-24","IIT 25-36")~"IIT 6+",TRUE~`Test Result/Outcome/Duration` )) %>%
  mutate(CoarseAgeGroup=gsub("^0([1-9])-0", "\\1-", CoarseAgeGroup)) %>%
  mutate(CoarseAgeGroup=case_when(CoarseAgeGroup %in% c("65-69","70-74","75+")~"65+",
                                  TRUE~CoarseAgeGroup)) %>%
  mutate(`Test Result/Outcome/Duration`=case_when(`Test Result/Outcome/Duration` %in%
                                                    c("6-12","13-24","25-36","36+")~"IIT 6+",
                                                  `Test Result/Outcome/Duration` %in%
                                                    c("<3")~"IIT <3",
                                                  `Test Result/Outcome/Duration` %in%
                                                    c("3-5") ~"IIT 3-5",
...
# BIND WITH REST ---------------------------------------------------------
#for partner review
tier_final_partner <- bind_rows(df_final_clean %>% select(District,all_of(partner_vars)),
